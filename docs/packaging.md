...
the committed asset and the drawing disagree. Change the mark and that test tells you to regenerate;
`BrandIcon.WriteIcoFile(path)` is what does it, over the sizes in `BrandIcon.IconSizes`.
